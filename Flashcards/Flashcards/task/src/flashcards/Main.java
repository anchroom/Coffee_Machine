...
public class Main {
    ...
    private static final String ASK_CARD_NAME = "The card:";
    private static final String ASK_CARD_DEFINITION = "The definition of the card:";
    private static final String THE_PAIR_HAS_BEEN_ADDED = "The pair (\"%s\":\"%s\") has been added.%n";
    private static final String THE_DEFINITION_ALREADY_EXISTS = "The definition \"%s\" already exists.%n";
    private static final String THE_CARD_ALREADY_EXISTS = "The card \"%s\" already exists.%n";
    private static final String CANT_REMOVE_THERE_IS_NO_SUCH_CARD = "Can't remove \"%s\": there is no such card.%n";
    private static final String THE_CARD_HAS_BEEN_REMOVED = "The card has been removed.%n";
    private static final String FILE_NAME = "File name:";
    private static final String N_CARDS_HAVE_BEEN_LOADED = "%d cards have been loaded.%n";
    private static final String FILE_NOT_FOUND = "File not found.%n";
    private static final String N_CARDS_HAVE_BEEN_SAVED = "%d cards have been saved.%n";
    ...
    // C:\Java\JetBrainsAcademy\Flashcards\Flashcards\task\capitals.txt
    ...
    /**
     * Стартует меню для выбора операции
     */
    private static void startMenu() {
        String operationType;
        do {
            printFormatMessage(MENU_MESSAGE);
    ...
    /**
     * Запрашивает ввоод текста пользователя
     *
    ...
    /**
     * Кейс добавления карточки в список.
     */
    private static void addCardToList() {
        String cardName = inputCardName(ASK_CARD_NAME);
        if (isNotContainsQuestByName(cardName)) {
            ...
            if (isNotContainsQuestByDefinition(cardDefinition)) {
                ...
                printFormatMessage(THE_PAIR_HAS_BEEN_ADDED, cardName, cardDefinition);
            } else {
                printFormatMessage(THE_DEFINITION_ALREADY_EXISTS, cardDefinition);
            }
        } else {
    ...
    /**
     * Кейс удаления карточки из списка
     */
    private static void removeCard() {
        String cardName = inputCardName(ASK_CARD_NAME);
        if (isNotContainsQuestByName(cardName)) {
            printFormatMessage(CANT_REMOVE_THERE_IS_NO_SUCH_CARD, cardName);
        } else {
            removeCardFromList(cardName);
            printFormatMessage(THE_CARD_HAS_BEEN_REMOVED);
        }

    }

    /**
     * Кейс запуска импорта из файла
     * Формат строки: "cardName:cardDefinition"
     */
    ...
    private static void importFile(String fileName) {
        try (Stream<String> stream = Files.lines(Paths.get(fileName))) {
            ...
            printFormatMessage(N_CARDS_HAVE_BEEN_LOADED, counter[0]);
        } catch (IOException ex) {
    ...
    /**
     * Кейс запуска экспорта в файл
     * Формат строки: "cardName:cardDefinition:errorCounter"
     */
    ...
    /**
     * Запуск экспорта данных из файла
     *
     * @param fileName - название файла
     */
    private static void exportFile(String fileName) {
        try (FileWriter fileWriter = new FileWriter(fileName)) {
            ...
            printFormatMessage(N_CARDS_HAVE_BEEN_SAVED, CARD_LIST.size());
        } catch (IOException ex) {
            ex.getCause();
        }
    }
    ...
    private static void runQuiz() {
        ...
        int quantity = Integer.parseInt(getUsersText());
    ...
    private static void runExit() {
        printFormatMessage(BYE_BYE);
    }

    /**
     * Кейс сброса статистики
     */
    ...
    /**
     * Кейс получения самого тяжелого вопроса
     */
    ...
    /**
     * Метод вывода форматированного текстового сообщения
     *
     * @param messageBody - шаблон выводимого сообщения. Один перенос строки %n добавляется всегда!
     * @param textValue   - атрибуты для заполнения шаблона
     */
    ...
    /**
     * Удаление карточки из списка по названию
     *
    ...
    /**
     * Запрос ввода названия карточки в консоли с текстом
     *
     * @param text - отображаемый текст запроса
    ...
    /**
     * Метод добавления карточки в список вопросов
     *
     * @param cardName       - название карточки
     * @param cardDefinition - описание карточки
     */
    ...
    /**
     * Проверка отсутствия карточки в списке вопросов по названию
     *
     * @param cardName - название карточки
     * @return - true - если карточки с таким именем нет в списке, иначе false
     */
    ...
    /**
     * Проверка отсутствия карточки в списке вопросов по описанию
     *
     * @param cardDefinition - описание карточки
     * @return - true - если карточки с таким именем нет в списке, иначе false
     */
...
/**
 * Сущность карточки с описанием.
 * cardName - название карточки
 * cardDefinition - описание карточки
 */
class Card {
